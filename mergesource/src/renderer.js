...
//hacky!
let multiplier = 1;//512 / 320;

class renderer_singleton{
	curr_sprites = [];
	draw_sprite = function(ctx,s){
		//expect horizontal sprites
		ctx.drawImage(s.src_img,s.curr_frame_index * s.frame_width,0,s.frame_width,s.src_img.height,(s.owner.rect.x_coord * multiplier),(s.owner.rect.y_coord * multiplier) - (camera_scroll_y * multiplier),s.owner.rect.width * multiplier,s.owner.rect.height * multiplier);
		s.next_frame();
	}
	render = function(){
		if(((new Date()) - last_interval) >= interval_ms) {
			...
			for(let s_itor = 0; s_itor < tile_map.length; s_itor++){
				...
				if(b.solid){
					...
					ctx.fillStyle = "black";
					ctx.fillRect((b.rect.x_coord * multiplier),(b.rect.y_coord * multiplier) - (camera_scroll_y * multiplier), b.rect.width * multiplier,b.rect.height * multiplier);
					ctx.fillStyle = "white";
				}
			}
